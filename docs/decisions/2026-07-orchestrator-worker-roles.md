...
| Select task worker backends, start/steer/interrupt workers, and handle quota/retry state     | PM              | Prefer one bounded continuation of the current viable Plan/Work thread before a fresh attempt.     |
...
- An incomplete current Plan or Work result is not automatically a failed attempt. The PM first
  continues its viable thread with precise bounded correction instructions; materially different,
  independently judged, context-recovery, capability-change, and post-newer-stage work use fresh
  attempts.
