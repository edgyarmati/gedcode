...
| Task-oriented chat lifecycle / Sidebar V2 (`32c6012d` and follow-ups)                                                | Potentially high, but the flat Inbox removed essential project/thread context in GedCode’s current workflow       | UI withdrawn after evaluation; keep the durable lifecycle foundation and preserved experiment branch for a future redesign      | Deferred                                        |
...
- Evaluated and withdrawn from the main UI: 2026-07-30.
- Notes: The flat two-category Inbox did not retain enough of GedCode’s established project and
  thread context to justify replacing the project-grouped Chat sidebar. Main keeps the durable
  Active/Snoozed/Settled projection foundation, restart-safe snooze wake-up, and automatic reopen
  behavior so a future design can reuse them. The current UI restores project-grouped Chat rows and
  keeps only the animated Chat/Orchestrator surface switch.
- Preserved implementation: branch `archive/inbox-ui-experiment`, including commit
  `37d0a5f30` with the later rich-row experiment. Treat that branch as design/reference work, not as
  the accepted mainline UI.
