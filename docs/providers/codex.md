...
This guide is for people who want to use more than one Codex account in GedCode.
...
In GedCode Settings, name it something obvious:
...
In GedCode Settings, add another Codex provider:
...
GedCode shows the authenticated email for providers that report one. Emails are blurred by default;
...
If you add a third Codex provider with a completely different `CODEX_HOME path`, GedCode treats it
