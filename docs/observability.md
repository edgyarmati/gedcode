...
GedCode has one server-side observability model:
...
Completed spans are written as NDJSON records to `serverTracePath` (by default, `~/.gedcode/userdata/logs/server.trace.ndjson`).
...
"/Applications/GedCode.app/Contents/MacOS/GedCode"
```
