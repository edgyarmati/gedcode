...
function getFavoriteButtonForModel(modelName: string) {
  const modelRow = Array.from(document.querySelectorAll<HTMLElement>('[role="option"]')).find(
    (row) => row.textContent?.includes(modelName),
  );
  const starButton = modelRow?.querySelector<HTMLButtonElement>('button[aria-label*="favorites"]');
...
describe("ProviderModelPicker", () => {
  ...
  it("toggles favorite stars when clicked", async () => {
    ...
    try {
      ...
      const favoriteButton = getFavoriteButtonForModel("Claude Opus 4.6");
      ...
      await vi.waitFor(() => {
        expect(getFavoriteButtonForModel("Claude Opus 4.6").getAttribute("aria-label")).toBe(
          expectedAriaLabel,
        );
      });
    } finally {
