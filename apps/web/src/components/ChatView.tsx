...
import { ThreadDetailLoadingState } from "./chat/ThreadDetailLoadingState";
...
export default function ChatView(props: ChatViewProps) {
  ...
  const activeThreadDetailReadiness = useStore(
    useMemo(
      () => (store) =>
        selectThreadDetailReadinessByRef(store, routeKind === "server" ? routeThreadRef : null),
      [routeKind, routeThreadRef],
    ),
  );
  ...
  const isInitialThreadDetailLoading =
    isServerThread &&
    (activeThreadDetailReadiness === "unrequested" || activeThreadDetailReadiness === "loading");
  ...
  const persistThreadSettingsForNextTurn = useCallback(
    async (input: {
      ...
      if (input.gedWorkflowEnabled !== (serverThread.gedWorkflowEnabled ?? true)) {
        await api.orchestration.dispatchCommand({
