...
/**
 * SQLite cannot expand the status CHECK constraint in place. Rebuild the
 * append-only projection table while preserving every existing run row, then
 * add durable review-resolution metadata for explicit commit/discard actions.
 */
export default Effect.gen(function* () {
  ...
  yield* sql`
    ...
    RENAME TO projection_project_context_runs_061
  `;
  yield* sql`
    CREATE TABLE projection_project_context_runs (
      ...
      resolution TEXT CHECK (resolution IS NULL OR resolution IN ('committed', 'discarded')),
      commit_hash TEXT,
  ...
  yield* sql`
    INSERT INTO projection_project_context_runs (
      ...
      result, failure_message, changes_json, scope_violation_paths_json, created_at, started_at,
      pending_review_at, failed_at, interrupted_at, updated_at
    )
    SELECT
      project_context_run_id, project_id, mode, tier, provider_instance_id, model,
      model_options_json, primary_checkout_path, schema_version, fingerprint, prompt,
      ...
      result, failure_message, changes_json, scope_violation_paths_json, created_at, started_at,
      pending_review_at, failed_at, interrupted_at, updated_at
    FROM projection_project_context_runs_061
  `;
  yield* sql`DROP TABLE projection_project_context_runs_061`;
