    ...
    execute: (row) =>
      sql`
        INSERT INTO projection_tasks (
          ...
          change_review_json,
          verification_json,
          no_changes_needed_json,
          landing_json,
          force_land_request_json,
        ...
        VALUES (
          ...
          ${JSON.stringify(row.dependsOnTaskIds)},
          ${row.supersedesTaskId},
          ${row.supersededByTaskId},
          ${row.cancellation === null ? null : JSON.stringify(row.cancellation)},
          ${row.changeReview === null ? null : JSON.stringify(row.changeReview)},
          ${row.verification === null ? null : JSON.stringify(row.verification)},
          ${row.noChangesNeeded === null ? null : JSON.stringify(row.noChangesNeeded)},
          ${row.landing === null ? null : JSON.stringify(row.landing)},
          ${row.forceLandRequest === null ? null : JSON.stringify(row.forceLandRequest)},
          ${row.releaseDispatch === null ? null : JSON.stringify(row.releaseDispatch)},
          ${JSON.stringify(row.roleCapabilityTiers)},
          ${row.playbookVersion},
          ${row.createdAt},
          ${row.updatedAt},
          ${row.archivedAt},
        ...
        DO UPDATE SET
          ...
          change_review_json = excluded.change_review_json,
          verification_json = excluded.verification_json,
          no_changes_needed_json = excluded.no_changes_needed_json,
          landing_json = excluded.landing_json,
          force_land_request_json = excluded.force_land_request_json,
