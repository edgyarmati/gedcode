...
it.layer(NodeServices.layer)("project-context run decider", (it) => {
  ...
  it.effect("enforces start and pending-review transitions with immutable changes", () =>
    Effect.gen(function* () {
      ...
      expect(Array.isArray(applyPlan)).toBe(false);
      const appliedEvent = Array.isArray(applyPlan) ? applyPlan[0] : applyPlan;
      expect(appliedEvent.type).toBe("project.context-run-applied");
