...
type VcsStatusSnapshotEvent = Extract<VcsStatusStreamEvent, { readonly _tag: "snapshot" }>;
...
describe("VcsStatusBroadcaster", () => {
  ...
  it.effect("streams a local snapshot first and remote updates later", () => {
    ...
    return Effect.gen(function* () {
      const broadcaster = yield* VcsStatusBroadcaster.VcsStatusBroadcaster;
      const snapshotDeferred = yield* Deferred.make<VcsStatusSnapshotEvent>();
  ...
  it.effect("delays automatic refresh when the cached remote snapshot is null", () => {
    const state = {
      currentLocalStatus: baseLocalStatus,
