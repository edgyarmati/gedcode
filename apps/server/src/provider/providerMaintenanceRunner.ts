...
>()("gedcode/provider/providerMaintenanceRunner") {}
            ...
            return yield* finish(
              makeUpdateState({
                ...
                message: couldNotVerify
                  ? "Update command completed, but GedCode could not verify the provider version."
                  : stillOutdated
                    ? "Update command completed, but GedCode still detects an outdated provider version."
                    : "Provider updated.",
                output: commandOutput(result),
              }),
            );
