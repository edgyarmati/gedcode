...
Use this when you want to connect to a GedCode server from another device such as a phone, tablet, or separate desktop app.
...
with the same server-side behavior as `gedcode serve --tailscale-serve`, then the server asks Tailscale
...
Use `gedcode serve --help` for the full flag reference. It supports the same general startup options as the normal server command, including an optional `cwd` argument.
...
Use this when you want the desktop app to start or reuse GedCode on another machine over SSH.
...
5. Confirm the launch. The desktop app probes the host, starts or reuses a remote GedCode server, opens a local port forward, and saves the environment.

After setup, the renderer connects to a local forwarded HTTP/WebSocket endpoint. The remote host still owns the actual GedCode server, projects, files, git state, terminals, and provider sessions.
...
The desktop SSH launcher connects with a non-interactive `sh` session, writes a small launcher script under `~/.gedcode/ssh-launch/<host-key>/`, starts or reuses a remote GedCode server, and forwards the remote loopback port back to your desktop.

The remote host must have a compatible Node.js runtime. GedCode uses the server package's `engines.node` requirement:

```text
^22.16 || ^23.11 || >=24.10
```

During SSH launch, GedCode first checks whether `node` is already available on `PATH`. If it is missing, the launcher tries common non-interactive shell locations and version-manager shims/activation hooks:
...
If launch fails with `node: command not found`, a port-scan failure, or a message that the remote Node version does not satisfy the required range, SSH into the host and check the same non-interactive shell path GedCode uses:
...
1. `gedcode serve` issues a one-time owner pairing token.
...
- `HOSTED_WEB_ROUTER_HOST`: host that receives channel-routing requests, such as
  `app.gedcode.example`
- `HOSTED_WEB_LATEST_ORIGIN`: latest-channel deployment origin
- `HOSTED_WEB_NIGHTLY_ORIGIN`: nightly-channel deployment origin
- `HOSTED_WEB_CHANNEL_COOKIE`: optional channel-selection cookie name

## Managing Access Later

Use `gedcode auth` to manage access after the initial pairing flow.
...
Use `gedcode auth --help` and the nested subcommand help pages for the full reference.
...
- Use `gedcode auth` to revoke credentials or sessions you no longer trust.
