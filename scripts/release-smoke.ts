...
function writeMacManifestFixtures(targetRoot: string): { arm64Path: string } {
...
files:
  - url: GedCode-9.9.9-smoke.0-arm64.zip
    sha512: arm64zip
    size: 125621344
  - url: GedCode-9.9.9-smoke.0-arm64.dmg
    sha512: arm64dmg
    size: 131754935
path: GedCode-9.9.9-smoke.0-arm64.zip
sha512: arm64zip
...
function writeWindowsManifestFixtures(
  ...
  writeFileSync(
    ...
    `version: 9.9.9-smoke.0
files:
  - url: GedCode-9.9.9-smoke.0-arm64.exe
    sha512: arm64exe
    size: 126621344
  - url: GedCode-9.9.9-smoke.0-arm64.exe.blockmap
    sha512: arm64blockmap
    size: 152344
path: GedCode-9.9.9-smoke.0-arm64.exe
sha512: arm64exe
releaseDate: '2026-03-08T10:32:14.587Z'
`,
  );
  ...
  writeFileSync(
    ...
    `version: 9.9.9-smoke.0
files:
  - url: GedCode-9.9.9-smoke.0-x64.exe
    sha512: x64exe
    size: 132000112
  - url: GedCode-9.9.9-smoke.0-x64.exe.blockmap
    sha512: x64blockmap
    size: 160112
path: GedCode-9.9.9-smoke.0-x64.exe
sha512: x64exe
...
try {
  ...
  assertContains(
    nightlyReleaseMetadata,
    "name=GedCode Nightly 9.9.10-nightly.20260413.321 (abcdef123456)",
    "Expected nightly metadata to include the short commit SHA in the release name.",
  );

  const { arm64Path } = writeMacManifestFixtures(tempRoot);
  const mergedManifest = readFileSync(arm64Path, "utf8");
  assertContains(
    mergedManifest,
    "GedCode-9.9.9-smoke.0-arm64.zip",
    "Mac manifest is missing the arm64 asset.",
  );
  assertMissing(
    resolve(dirname(arm64Path), ["latest", "mac", "x64.yml"].join("-")),
    "Release smoke unexpectedly created an Intel macOS manifest.",
  );
  ...
  assertContains(
    ...
    "GedCode-9.9.9-smoke.0-arm64.exe",
    "Merged Windows manifest is missing the arm64 asset.",
  );
  assertContains(
    mergedWindowsManifest,
    "GedCode-9.9.9-smoke.0-x64.exe",
