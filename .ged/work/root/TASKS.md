...
| CHAT-FORK-01 | NEXT | Add the typed server/RPC fork operation. Codex uses native `thread/fork`, then rolls back only the fork when an earlier completed turn is selected; unsupported providers or boundaries use copied visible history in a fresh session. Retain current filesystem state and never mutate the source. | Contract/provider/integration tests cover latest and earlier Codex turns, copied-history fallback, source immutability, and explicit filesystem semantics. |
