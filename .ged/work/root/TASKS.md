...
| ORCH-VERIFY-FINALIZE-01 | DONE | Finalize verifier-owned documentation outside the Codex sandbox: audit dirty paths, create a server-owned evidence commit, and record verification only against the resulting clean exact HEAD. Surface non-documentation residue for PM resolution instead of wedging the task. | Focused real-Git settlement, decider, projection, PM recovery, and runtime tests prove allowed verifier docs commit atomically, disallowed paths never commit, failures remain recoverable, and verification binds to the committed HEAD. |
...
| ID | Status | Slice | Verification |
| --- | --- | --- | --- |
| ORCH-STAGES-01 | DONE | Make every Work/Verify attempt in task history selectable, persist the selected stage thread in the task URL, highlight it, default to the active/latest attempt, and repair the narrow detail-rail gate/diff layout. | Focused route/timeline logic and Chromium coverage prove old attempts remain readable, deep links survive refresh, invalid selections recover, and narrow layouts do not clip controls. |

## P1 — Landing Retry Feedback
...
## P0 — Durable PM Lifecycle and Landing Automation

| ID | Status | Slice | Verification |
| --- | --- | --- | --- |
| ORCH-LIFECYCLE-01 | DONE | Add a typed, append-only, per-PM-thread lifecycle inbox with idempotency keys, durable pending/delivered state, and server-side wake dispatch. Merge pending lifecycle context into an already-queued user PM message; otherwise dispatch it after the current PM turn settles. | Contract, decider, persistence, replay, queue, and runtime tests prove dedupe, restart recovery, user-message precedence, no synthetic user messages, and no active-turn interruption. |
| ORCH-LIFECYCLE-02 | DONE | Retry a helper exactly once only when its provider explicitly reports a transient transport failure. Retain its helper ID/thread, leave quota/auth/capability/provider outcomes terminal, and use the existing helper completion/failure lifecycle inbox for the durable final wake. | Focused helper-reactor and PM-runtime tests cover one retry, same identity/thread, second-failure finalization, no retry for quota/auth/permission/provider failures, and durable PM re-entry. |
| ORCH-THREAD-OWNERSHIP-01 | DONE | Persist explicit ownership links when creating every new PM, stage, and helper thread; filter those threads from normal Chat and expose all attempts/helpers chronologically from task history. Do not migrate or classify existing threads. | Schema/projection/API and focused server/web tests prove creation-time metadata, Chat exclusion, latest-attempt selection, chronological helper labels, and unchanged legacy thread visibility. |
| ORCH-LAND-APPROVAL-01 | DONE | Make verified landing approval immediately create or update the stored draft PR proposal, remove the redundant normal-flow Land control, and retain retryable landing failure. Invalidate approval whenever verified HEAD changes. | Decider/landing/reactor and Chromium tests cover approval-to-landing progression, idempotent PR update, failure/retry presentation, exact-HEAD invalidation, and no implicit ready/merge action. |
| ORCH-PR-LIFECYCLE-02 | DONE | Track orchestrator-created open PRs and synchronise them through conditional, adaptive non-LLM GitHub requests. Append replay-safe `pr_opened`/`pr_merged` lifecycle events, wake the PM on merge, and unblock dependents only after merge. | GitHub-client, scheduler, persistence/replay, dependency, and runtime tests cover active/background cadence, no tracked-PR stop, conditional response handling, external merge, duplicate delivery, restart, and unavailable GitHub recovery. |
| ORCH-WORKER-SANDBOX-01 | DONE | Add the global default-on worker-network setting and enforce effective Codex worker `workspace-write` network policy: PM handoffs can restrict it but never override a global disable. | Settings/contracts, provider launch, prompt, and runtime tests pin default-on, global-off precedence, per-handoff restriction, explicit-stage-only scope, and unchanged PM permission policy. |
| ORCH-WORKER-CAPABILITY-02 | DONE | Model capability-bound work as a durable paused stage that retains its worker session. Let the PM complete or seek approval for the narrow operation, append its result, and resume/steer the same worker; expire only on timeout or explicit cancellation. | Decider/projection, provider-runtime, PM-tool, approval, restart, timeout, and Chromium tests prove session retention, one resume, denial/retry paths, no automatic escalation, and visible paused state. |
| ORCH-LIFECYCLE-VERIFY-01 | DONE | Add cross-feature recovery tests, operational documentation, and release-note coverage for inbox delivery, thread visibility, approval-to-PR, merge synchronization, and paused workers. | Focused end-to-end/restart scenarios cover the full flow; `CHANGELOG.md`, docs, `bun fmt`, `bun lint`, relevant package typechecks, and targeted `bun run test` commands pass. |
