...
| ORCH-LC-03 | NEXT | Reconcile cancellation interrupted by server restart and finish remaining shutdown/cleanup steps exactly once. | Restart integration test resumes pending cancellation without duplicate interrupts or cleanup. |
| ORCH-LC-04 | TODO | Add restart reconciliation for interrupted/orphaned active stages and permit a clean retry. | Restart test leaves no stale active stage and a new handoff succeeds. |
| ORCH-LAND-01 | TODO | Add guarded `landTask` executor to PM tools and MCP using the existing `task.land` decider invariant. | PM/MCP tests dispatch `task.land` only after approved land gate. |
