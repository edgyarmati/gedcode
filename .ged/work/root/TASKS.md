...
| ORCH-PMBOOT-02 | NEXT | Expose Claude's read-only Skill loader to PM sessions and replace impossible native-subagent guidance with bounded orchestration worker handoffs. | Provider policy tests allow Skill while continuing to deny Task/Agent and mutation tools; prompt tests require worker handoff guidance. |
