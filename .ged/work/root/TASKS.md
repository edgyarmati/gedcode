...
| CHAT-GED-02 | NEXT | Restore the Normal/GED composer selector and draft/new-thread plumbing for normal chat using the lightweight mode contract. Add a global default-for-new-threads setting (GED on by default) and an explanatory selector tooltip. | Logic and Chromium tests cover the global default, tooltip, selection, persistence, send/new-thread propagation, and unchanged Normal prompts. |
