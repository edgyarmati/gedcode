...
| ORCH-ROLES-02 | NEXT | Extend the shared per-role backend picker at project and task scope to expose provider instance (harness), model, and supported thinking/reasoning level while preserving valid model options across changes. | Logic and Chromium tests cover inheritance, custom instances, model-dependent effort choices, stale-option cleanup, persistence, effective selection, and task overrides. |
