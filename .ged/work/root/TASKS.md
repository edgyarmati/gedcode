...
| PROJECT-CTX-01 | NEXT | Define canonical project-context files and a scanner for missing, whitespace-only, template-only, and substantive content. Persist per-project schema version, fingerprint, dismissal, and completion. | Filesystem/persistence tests cover every classification, material changes, schema upgrades, and `.gedcode/` exclusion. |
