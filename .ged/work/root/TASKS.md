...
| ORCH-TIER-02 | NEXT | Persist a one-time manual preset-migration requirement for legacy global/project role selections and block every Orchestrator entry/API until complete. | Migration/state tests enumerate legacy selections, reject partial mappings, persist completion, and prove no bypass after restart. |
