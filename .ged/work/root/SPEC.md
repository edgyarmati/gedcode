...
4. Default steering selects only `currentStageThreadId`, never the latest historical worker thread.
5. Rejection dispatches no provider turn and explains that a fresh tracked attempt is required.
6. Focused tests, formatting, lint, and server typecheck pass.
